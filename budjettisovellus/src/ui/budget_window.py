...
class BudgetView():
    def __init__(self, root, handle_logout, purchases_view):
        ...
        self._colors = ["purple", "orange", "blue", "grey",
                        "yellow", "pink", "red", "brown", "white"]
...
class PurchasesView():
    def __init__(self, root, go_back, handle_add_purchase):
        ...
        self._categories = ("Appointments", "Bills", "Groceries", "Leisure",
                            "Necessities", "Restaurant", "Shopping", "Subscriptions", "Other")
    ...
    def purchases_view(self):
        ...
        self._set_new_budget_label = ttk.Label(
            master=self._frame, text="Set new budget:")
        ...
        self._category_entry = ttk.OptionMenu(
            self._frame, self._variable, self._categories[0], *self._categories)
        ...
        add_purchase_button = ttk.Button(
            master=self._frame, text="Add", command=self._add_process)
        ...
        purchases_list.grid(row=9, column=0, padx=5,
                            pady=5, sticky=constants.EW)

        purchases_scrollbar = Scrollbar(master=self._frame)
        purchases_scrollbar.grid(
            row=9, column=1, padx=5, pady=5, sticky=(constants.NS, constants.W))
        ...
        delete_purchases_button = ttk.Button(
            master=self._frame, text="Delete purchases", command=self._handle_deletions)
